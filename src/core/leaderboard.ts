...
export const getBoardsList = async(source: LX.OnlineSource) => {
  ...
  const board = await requestWithRetry<Board>({
    // eslint-disable-next-line @typescript-eslint/promise-function-async
    factory: () => (musicSdk[source]?.leaderboard.getBoards() as Promise<Board>),
    validate: result => Array.isArray(result?.list) && result.list.length > 0,
  })
...
const getListLimit = async(source: LX.OnlineSource, bangId: string, page: number): Promise<ListDetailInfo> => {
  ...
  const requestPage = async(sourcePage: number): Promise<ListDetailInfo> => {
    return requestWithRetry<ListDetailInfo>({
      // eslint-disable-next-line @typescript-eslint/promise-function-async
      factory: () => (musicSdk[source]?.leaderboard.getList(bangId, sourcePage) ?? Promise.reject(new Error('source not found'))),
      validate: result => Array.isArray(result?.list) && Number.isFinite(result.total),
    })
  }

  return requestPage(sourcePage + 1).then((result: ListDetailInfo) => {
    if (listCache !== cache.get(listKey)) throw new Error('list changed')
