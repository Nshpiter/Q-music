...
import { getMusicAccountMusicUrl } from '@/core/musicAccount'
...
/** 音质降级顺序，对齐桌面端 QUALITY_FALLBACKS 的可移植子集 */
const QUALITY_FALLBACK_ORDER: LX.Quality[] = ['flac24bit', 'flac', '320k', '128k']

/**
 * 构建音质降级候选链：目标音质优先，其后按降级顺序过滤出该歌曲可用且音源支持的档位
 */
export const getPlayQualityCandidates = (quality: LX.Quality, musicInfo: LX.Music.MusicInfoOnline): LX.Quality[] => {
  const startIndex = QUALITY_FALLBACK_ORDER.indexOf(quality)
  const lowerQualitys = startIndex < 0 ? [] : QUALITY_FALLBACK_ORDER.slice(startIndex + 1)
  const list = global.lx.qualityList[musicInfo.source]
  return [
    quality,
    ...lowerQualitys.filter(q => musicInfo.meta._qualitys[q] && list?.includes(q)),
  ]
}
...
export const handleGetOnlineMusicUrl = async({ musicInfo, quality, onToggleSource, isRefresh, allowToggleSource }: {
  ...
  // 官方账号线路优先（仅 tx/wy；选择自定义 API 时跳过，避免改变普通用户的线路预期）
  const isCustomApi = Boolean(settingState.setting['common.apiSource'])
  if (!isCustomApi && (musicInfo.source == 'tx' || musicInfo.source == 'wy')) {
    const songId = String(musicInfo.meta.songId ?? musicInfo.id)
    const mediaId = musicInfo.source == 'tx' ? musicInfo.meta.strMediaMid : musicInfo.id
    const official = await getMusicAccountMusicUrl(musicInfo.source, songId, mediaId, targetQuality, isRefresh)
    if (official.status == 'available' && official.url) {
      return { musicInfo, url: official.url, quality: official.quality, isFromCache: false }
    }
  }

  // 同源内逐档降级尝试，全部失败后再走跨源换源
  const candidateQualitys = getPlayQualityCandidates(targetQuality, musicInfo)
  let lastError: any = new Error('get music url failed')
  for (const candidate of candidateQualitys) {
    let reqPromise: Promise<{ url: string, type: LX.Quality }>
    try {
      reqPromise = musicSdk[musicInfo.source].getMusicUrl(toOldMusicInfo(musicInfo), candidate).promise
    } catch (err: any) {
      console.log(err)
      if (err.message == requestMsg.tooManyRequests) throw err
      lastError = err
  ...
  if (!allowToggleSource) throw lastError
  onToggleSource()
  // eslint-disable-next-line @typescript-eslint/promise-function-async
