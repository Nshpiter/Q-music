...
const DefaultBar = memo(() => {
  ...
  return <View style={{
    ...styles.progressBar,
    backgroundColor: theme['q-outline'],
    position: 'absolute',
    width: '100%',
    left: 0,
    top: 0,
...
const BufferedBar = memo(({ progress }: { progress: number }) => {
  ...
  const theme = useTheme()
  return <View style={{ ...styles.progressBar, backgroundColor: theme['c-primary-alpha-700'], position: 'absolute', width: `${progress * 100}%`, left: 0, top: 0 }}></View>
})
        ...
        <BufferedBar progress={buffered} />
        {
          draging
            ? (
                <>
                  <View style={{ ...styles.progressBar, backgroundColor: theme['q-accent'], width: progressStr, position: 'absolute', left: 0, top: 0 }} />
                  <View style={{ ...styles.progressBar, backgroundColor: theme['c-primary-light-100'], width: `${dragProgress * 100}%`, position: 'absolute', left: 0, top: 0 }} />
                </>
              ) : (
                <View style={{ ...styles.progressBar, backgroundColor: theme['q-accent'], width: progressStr, position: 'absolute', left: 0, top: 0 }} />
              )
