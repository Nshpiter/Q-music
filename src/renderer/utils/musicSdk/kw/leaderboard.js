...
export default {
  ...
  getUrl(id, page) {
    return `https://kbangserver.kuwo.cn/ksong.s?from=pc&fmt=json&pn=${page - 1}&rn=${this.limit}&type=bang&data=content&id=${id}&show_copyright_off=0&pcmp4=1&isbang=1`
  },
  filterData(rawList) {
  ...
  filterLegacyData(rawList) {
    return rawList.map(item => {
      ...
      return {
        ...
        songmid: item.id,
        source: 'kw',
        interval: formatPlayTime(parseInt(item.song_duration || item.duration)),
        img: item.pic ?? '',
        lrc: null,
        otherSource: null,
        types,
  ...
  getList(id, page, retryNum = 0) {
    ...
    return this.getData(this.getUrl(id, page)).then(({ statusCode, body }) => {
      if (statusCode !== 200 || !Array.isArray(body.musiclist)) return this.getList(id, page, retryNum)

      const total = parseInt(body.num) || body.musiclist.length
      const list = this.filterLegacyData(body.musiclist)
