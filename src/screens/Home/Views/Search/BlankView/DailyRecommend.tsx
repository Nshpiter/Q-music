...
import { Q_TOUCH_HIT_SLOP, Q_UI, qSurfaceShadow } from '@/theme/ui'
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.actions}>
          <Button accessibilityLabel={t('search_daily_recommend_refresh')} disabled={loading} style={{ ...styles.action, backgroundColor: theme['q-surface-tint'] }} onPress={() => { void load(sourceRef.current, true) }}>
            <Text size={12} color={theme['q-accent-text']}>{t('search_daily_recommend_refresh')}</Text>
          </Button>
          <Button accessibilityLabel={t('search_daily_recommend_play')} disabled={loading || !list.length} style={{ ...styles.action, backgroundColor: theme['q-accent'] }} onPress={() => { void handlePlay() }}>
        ...
        : loadError
          ...
          : list.length
            ? list.slice(0, 8).map((item, index) => (
                <Button key={item.id} accessibilityLabel={`${item.name} · ${item.singer}`} style={styles.item} onPress={() => { void handlePlay(index) }}>
                  <View style={{ ...styles.cover, backgroundColor: theme['q-surface-tint'] }}>
                    {item.meta.picUrl
                      ? <Image source={{ uri: item.meta.picUrl }} style={styles.coverImage} />
                      : <Icon accessible={false} name="album" color={theme['q-accent-text']} rawSize={18} />}
                  </View>
                  ...
                  <View style={{ ...styles.playFace, backgroundColor: theme['q-surface-tint'] }}>
                    <Icon accessible={false} name="play" color={theme['q-accent-text']} rawSize={13} />
                  </View>
                </Button>
            ))
