...
- QQ 音乐与网易云音乐歌曲优先尝试平台官方播放地址，并按目标音质逐级降级。
- 官方地址不可用、版权受限或账号权限不足时，自动回退到现有自定义接口与跨源匹配。
- 官方地址使用独立短时缓存，不覆盖既有音乐源脚本与持久化接口缓存。
- 播放详情页提供标准、HQ、无损与 Hi-Res 快捷切换，并在换音质时保留播放进度。

## 榜单与来源标识

- 排行榜请求增加有限重试、响应校验与失效榜单恢复。
- 在线歌曲在列表、搜索与榜单中使用各平台官方图标标识来源。
